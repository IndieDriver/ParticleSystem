...
__kernel void clinit(float4 cursor, __global float4 *lpos, __global float4 *lcol, __global float4 *lvel)
{
    ...
    float r = (float)get_random_0_1_ul(&seed)*(0.3f);
...
__kernel void clpart(float4 cursor, __global float4 *lpos, __global float4 *lcol, __global float4 *lvel)
{
    ...
    //float length = fast_length(force);
    //float acc = G * force / (length * length * length);
...
/*
    ...
    curtest.x = cursor.x;
    curtest.y = cursor.y;
    /*if (fast_distance(cursor, pos) < 0.1f)
    {
    col.x = 1.0f;
    col.y = 0.0f;
    col.y = 0.0f;
